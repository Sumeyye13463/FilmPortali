...
{
    public partial class FrmYönetmenKayit : Form
    {
        public FrmYönetmenKayit()
        {
            InitializeComponent();
        }
        ...
        private void textBox1_TextChanged(object sender, EventArgs e) //BİYOGRAFİ TEXTBOX'U
        {
        ...
        private void yKaydet_Click(object sender, EventArgs e)
        {
            ...
            if (tYonetmen_A.Text != "" && tBiyografi.Text != "" && resimYolu != "" && cinsiyet != "")
            {
                MessageBox.Show("Kayıt işlemlerinizi gerçekleştirebilirsiniz.");

                connection.Open();

                string adSoyad = tYonetmen_A.Text.ToString().ToUpper();
                //insert into tabloAdi (alanlar) values (alanlara gelecek değerler)
                SqlCommand kayit = new SqlCommand("insert into Yonetmenler (Y_AdSoyad, Y_Cinsiyet, Y_Yas, Y_Biyografi, Y_Resmi) values (@p1, @p2, @p3, @p4, @p5)", connection);
        ...
        private void FrmYönetmenKayit_Load(object sender, EventArgs e)
        {
